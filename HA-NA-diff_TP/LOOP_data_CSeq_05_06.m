...
%     clc
    clearvars -except tim CSeq REF1 SZ CSeq

    if tim == 1
    seq = fastaread('NA_05_06.fasta');  tgseq = 470; %% for consensus
    elseif tim == 2
...
%%
        ...
        if tim ==  1 
        multialignwrite('full_align_NA_05_06aa.aln', ma)
    [Headers, Sequences] = multialignread('full_align_NA_05_06aa.aln');
    elseif tim == 2
        multialignwrite('full_align_NA_05_08aa.aln', ma)
   ...
   for j= 1:pos
      ...
      if size(setdiff(SEQ(j),REF1(j)),2) == 1         
        ...
        K(i,j) = 1;
             ...
             if tim == 1  
    disp(CSeq(271:275))%save('Data_NA_05_06.mat', 'K', 'pos', 'Seq', 'tree')  % NA 4 time-points data
    elseif tim == 2
    save('Data_NA_05_08aa.mat', 'K', 'pos', 'Seq', 'tree')
    else;    end
