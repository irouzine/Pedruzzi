...
yticks([]), yticklabels('')
  ...
  subplot(2,2,2), sub = plot(Gsub,'NodeColor',[.1 .1 .1],'EdgeAlpha',0.5, 'NodeLabel',[], 'EdgeLabel',[], 'LineWidth',2); axis square, hold on, % separate each subclusters (greater than two sites)
xlabel('Raw UFE_{AB}')
set(gca,'visible','off')
...
% Res = Results(:,[3,4]); Res(Res == 0) = NaN;
%  Res(any(isnan(Res), 2), :) = [];
%  
%                                                                                     subplot(2,2,3), 
% scatter(Res(:,1),Res(:,2),'g','LineWidth',2), axis square,
