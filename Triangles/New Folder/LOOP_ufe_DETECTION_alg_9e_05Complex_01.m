...
Dat_run(a:b,:,:) = DAt(:,:,:);
                                              ...
                                              str = sprintf('Th=%g, E=%g, muL=%g, s_0=%g, f_0=%g  \n N=%d, L=%g, t=%g,  POPs=%g, \n TOPOLOGY #%g ', th, E_st, U, s0, f0/L, N, L, td, runn, tp); %disp('a=0, pos epi.'),
...
if AVG == 1
    ...
    subplot(2,2,1), sub = plot(Gsub,'NodeColor','k','EdgeColor','k','EdgeAlpha',0.5, 'NodeLabel',[], 'EdgeLabel',[],'LineWidth',2); axis square, hold on, % separate each subclusters (greater than two sites)
xlabel('Raw UFE_{AB}'), set(gca,'visible','off'), set(gca, 'FontSize', 16), xticks([]), xticklabels(''), yticks([]), yticklabels('')

    subplot(2,2,2), sub2 = plot(Gsub,'NodeColor',[.5 .5 .5],'EdgeColor',[.5 .5 .5],'EdgeAlpha',0.5, 'NodeLabel',[], 'EdgeLabel',[],'LineWidth',2); axis square, hold on, % separate each subclusters (greater than two sites)
xlabel('Raw UFE_{AB}'), set(gca,'visible','off'), set(gca, 'FontSize', 16), xticks([]), xticklabels(''), yticks([]), yticklabels('')
Gsub_2 = Gsub;

else
    
  nP = size(Gsub_2.Edges,1);
  
  Edg = table2cell(Gsub_2.Edges); 
  
    for isi=1:nP   
     
    ip1 = Edg{isi,1}{1,1}; ip1 = str2double(ip1);
    ip2 = Edg{isi,1}{1,2}; ip2 = str2double(ip2);
        
f00 = mean(all(Ko(:,[ip1, ip2]) == ones(Ng,1)*[0 0],2)); f01 = mean(all(Ko(:,[ip(i,1), ip(i,2)]) == ones(Ng,1)*[0 1],2));
f10 = mean(all(Ko(:,[ip1, ip2]) == ones(Ng,1)*[1 0],2)); f11 = mean(all(Ko(:,[ip(i,1), ip(i,2)]) == ones(Ng,1)*[1 1],2));
 
coor(isi,:) = [ip1,ip2];

UFE_2(isi) = 1 - (log(f11/f00))/((log(f01*f10/f00^2)));
...
% for BLUE coloring (after averaging)
...
Bt_u = unique(Bt,'Stable'); Bt_0 = Bt; Bt_0(:) = 0;
Cb = table2cell(Gsub_2.Nodes)';

for uub = 1 : length(Bt_u)
    
indexB = find(strcmp(num2str(Bt_u(uub)), Cb));
indB = find(Bt == Bt_u(uub));
Bt_0(1,indB) = indexB; 
end

highlight(sub2,[Bt_0],'NodeColor','b','EdgeColor','b','LineWidth',2)
...
disp(Results) 
                                                                                    ...
                                                                                    subplot(2,2,3), 
scatter(Results(:,3),Results(:,4),'filled'), axis square, xlabel('UFE_{ij}'), ylabel('UFE_{ij0}'), hold on,
set(gca,'FontSize', 16,'linewidth',2)

% threeshold kept to 2 by default:
...
GsuB.Edges.Weight = UFEiii_min;
                                                                               ...
                                                                               subplot(2,2,4), 
h = plot(GsuB,'NodeColor',[.1 .1 .1],'EdgeColor','r','EdgeAlpha',0.5, 'NodeLabel',[], 'EdgeLabel',[],'LineWidth',2); xlabel('min(UFE_{AB0})'), axis square, hold on % separate each subclusters (greater than two sites)
set(gca, 'FontSize', 16)
xticks([]), xticklabels('')
yticks([]), yticklabels('')
set(gca,'visible','off')
...
highlight(h,[Tt_0],'EdgeColor','g','LineWidth',2)
end
