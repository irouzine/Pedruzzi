...
Dat_run(a:b,:,:) = DAt(:,:,:);
                                              ...
                                              str = sprintf('Th = %g, E = %g, muL = %g, s_0 = %g, f_0 = %g  \n N = %d, L = %g, t = %g,  # of populations = %g, \n Double-arches topology', th, E_st, U, s0, f0/L, N, L, td, runn); %disp('a=0, pos epi.'),
...
if AVG == 1
figure(1), sub = plot(Gsub,'NodeColor','k','EdgeColor','k','EdgeAlpha',0.3, 'NodeLabel',[], 'EdgeLabel',[],'LineWidth',2); axis square, hold on, % separate each subclusters (greater than two sites)
xlabel('Raw UFE_{AB}')
set(gca,'visible','off')
...
figure(2), sub = plot(Gsub,'NodeColor',[.1 .1 .1],'EdgeColor','k','EdgeAlpha',0.5, 'EdgeFontSize', 9, 'NodeLabel',[], 'EdgeLabel',Gsub.Edges.Weight, 'LineWidth',2); axis square, hold on, % separate each subclusters (greater than two sites)
xlabel('Raw UFE_{AB}')
...
disp(Results) 
                                                                                    ...
                                                                                    figure(3), 
subplot(1,2,1), scatter(Results(:,3),Results(:,4),'MarkerEdgeColor','k','MarkerFaceColor','r'), axis square, xlabel('UFE_{ij}'), ylabel('UFE_{ij0}'), hold on,
...
scatter(Res(:,1),Res(:,2),'MarkerEdgeColor','k','MarkerFaceColor','g'), hold on

set(gca,'FontSize', 16,'linewidth',2)
a = annotation('textbox', [0.75, 0.1, 0.1, 0.1], 'String', str, 'EdgeColor', 'w');
a.FontSize = 14;
dim = [.2 .74 .25 .15];
...
hold on
plot([th th],y1,'r', 'LineWidth', 2), hold on

cc = [.7 .7 .7];

line(x,.5*x,'Color',cc,'LineStyle','--', 'LineWidth', 2), hold on
line(x,x,'Color',cc,'LineStyle','--', 'LineWidth', 2), hold on
...
GsuB.Edges.Weight = UFEiii_min;
                                                                             ...
                                                                             figure(4) 
h = plot(GsuB,'NodeColor',[.5 .5 .5],'EdgeColor','r','EdgeAlpha',0.6, 'EdgeFontSize', 9, 'NodeLabel',[], 'EdgeLabel',GsuB.Edges.Weight,'LineWidth',2); xlabel('min(UFE_{AB0})'), axis square, hold on % separate each subclusters (greater than two sites)
set(gca, 'FontSize', 16)
xticks([]), xticklabels('')
yticks([]), yticklabels('')
set(gca,'visible','off')
...
% highlight(h,Tt_0)
highlight(h,Tt_0,'EdgeColor','g','LineWidth',3, 'NodeColor', 'k')
...
% 
% fig_n1= ['fig_2a.pdf']; fig_n2= ['fig_2b.pdf']; 
% fig_n3= ['fig_2c.pdf']; %fig_n4= ['fig_2d.pdf'];
% 
