...
% stat = horzcat(ip,fhap,UFE, UFEiii(:,:));

     stat_ = horzcat(ip,UFE, UFEiii(:,:));
     
     
     stat0 = mean(UFEiii,2);
stat_0 = horzcat(ip,UFE, stat0);
...
[I,J] = find(Ee); 
 ...
 tUF = prctile(stat_r(:,7),80);    tUF_t(t) = tUF;
   
  % ----------------------------------------
  
  
[N2] = find(stat_0(:,3) > tUF);[N3] = find(stat_0(:,4) > tUF1);[N4] = find(stat_p(:,1) > tUF2);
